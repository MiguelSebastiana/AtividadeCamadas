...
public class ContatoService {
    ...
    public Contato atualizarContatoo(Contato contato) throws SQLException {

        if(contatoRepository.existePorNome(contato.getNome())){
            throw new RuntimeException("O contato já foi cadastrado");
        }

        return contatoRepository.upadteContato(contato);

    }

    public Contato deletarContatoo(long id) throws SQLException {

        if(contatoRepository.existePorId(()))
    }
}
